...
- ci(windows): size the core wave budget from its manifest and enforce it
...
- feat(providers): derive per-worker gateway URL from backend nickname
...
- fix(ci): mktemp-scope-lint sees 'command mktemp' sites
...
- fix(duo): end reviewer turn after plan approval
...
- test(bg-run): drop inherited session scratch vars in the shared harness
- chore: trim redundant developer and reviewer prompt text
...
- test(inventory): warn on timing drift and lane backstop mismatch
- test: bind reclaim to record/root identity behind a final destructive gate
- chore(config): pretty-print default backends registry
...
- test: split explore suites into a an internal path pyramid
...
- fix(test): override the systemd registrar seam in the baton self-start test
...
- feat(explore/live): add lean startup profile for Windows local driver
...
- feat(ci): surface suite trend and escalation in lane tracker updates
...
- fix: warn before slow next-work startup
...
### Performance
- perf(console): hold one fleet snapshot instead of re-running the CLI per request

### Other Changes
- ci: add a console-only fast merge lane
